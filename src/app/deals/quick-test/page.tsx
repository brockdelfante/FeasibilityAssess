...
export default function QuickTestPage() {
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto space-y-6">
      ...
      <p className="text-gray-500">Simplified feasibility check for immediate viability screening.</p>
        ...
        <Card className="bg-blue-50 border-blue-200">
          ...
          <CardContent className="space-y-6">
            <div className="flex justify-between items-end border-b pb-2">
              <span className="text-gray-600">Total Profit (Surplus)</span>
              <span className="text-2xl font-bold text-blue-900">{formatCurrency(profit)}</span>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <p className="text-xs text-gray-500 uppercase">Return on Cost</p>
                <p className={`text-lg font-bold ${roc >= 0.2 ? "text-green-600" : "text-red-600"}`}>
                  {(roc * 100).toFixed(1)}%
                </p>
              </div>
              <div className="space-y-1">
                <p className="text-xs text-gray-500 uppercase">Senior LVR</p>
                <p className={`text-lg font-bold ${lvr <= 0.65 ? "text-green-600" : "text-red-600"}`}>
                  {(lvr * 100).toFixed(1)}%
                </p>
              </div>
            </div>
